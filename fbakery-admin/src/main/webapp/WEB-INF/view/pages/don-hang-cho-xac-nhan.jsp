...
<jsp:include page="../../includes/header.jsp">
	<jsp:param name="title" value="Admin - Order Loading Accept" />
</jsp:include>
										...
										<tbody>
											...
											<c:forEach var="item" items="${list }">
												<c:url var="loadUpdateLink"
													value="cap-nhat-thong-tin-hoa-don-cho-xac-nhan">
													<c:param name="orderID" value="${item.maHD}" />
												</c:url>
												<c:url var="loadOrderDetail" value="chi-tiet-hoa-don">
													<c:param name="orderID" value="${item.maHD}" />
												</c:url>
												<c:url var="delOrderDetail" value="xoa-hoa-don">
													<c:param name="orderID" value="${item.maHD}" />
												</c:url>
												<tr>
													<td>${count }</td>
												...
												<tr>
													<td><a href="${loadUpdateLink }"
														class="btn btn-link p-0 align-items-center"
														onclick="if(!(confirm('Bạn có chắc xác nhận đơn hàng này?'))) return false;">Xác
															nhận</a></td>
													<td><a href="${loadOrderDetail}"
														class="btn btn-link p-0">Xem chi tiết hóa đơn</a></td>

													<td><a href="${delOrderDetail }"
														class="btn btn-link p-0 text-danger"
														onclick="if(!(confirm('Bạn có chắc muốn xóa đơn hàng này?'))) return false;">Xóa
													</a></td>
												</tr>
